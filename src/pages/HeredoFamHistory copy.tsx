...
function DraggablePathologyTag({ patology, onRemove, isFromCatalog = false }: DraggablePathologyTagProps) {
  ...
  // Estilo base para el elemento arrastrable
  const draggableStyle = {
    transform: transform ? `translate3d(${transform.x}px, ${transform.y}px, 0)` : undefined,
    zIndex: isDragging ? 100 : 1,
    opacity: isDragging ? 0 : 1, // Ocultamos el original durante el arrastre
    transition: 'opacity 0.1s ease',
  };

  // Estilo para el elemento que se muestra durante el arrastre (usando el wrapper de DnD)
  const combinedStyle = {
    ...draggableStyle,
    backgroundColor: isFromCatalog 
      ? `${currentTheme.colors.primary}20`
      : currentTheme.colors.background,
    color: isFromCatalog 
      ? currentTheme.colors.primary 
      : currentTheme.colors.text,
    border: `1px solid ${isFromCatalog 
      ? `${currentTheme.colors.primary}50`
      : currentTheme.colors.border}`,
  };

  // Estilo para el elemento que se arrastra (el clon)
  const dragOverlayStyle = {
    ...
    color: isFromCatalog 
      ? currentTheme.colors.primary 
      : currentTheme.colors.text,
    border: `1px solid ${isFromCatalog ? `${currentTheme.colors.primary}50` : currentTheme.colors.border}`,
    boxShadow: `0 4px 6px -1px ${currentTheme.colors.primary}20, 0 2px 4px -1px ${currentTheme.colors.primary}10`,
    opacity: 0.9,
    // Asegurar que el elemento arrastrado mantenga el mismo aspecto visual
  ...
  return (
    <div
      ref={setNodeRef}
      style={isDragging ? { ...combinedStyle, visibility: 'hidden' } : combinedStyle}
      {...attributes}
      {...listeners}
      className={clsx(
        'flex items-center gap-1 px-3 py-1 text-sm rounded-md touch-none select-none',
        'cursor-grab active:cursor-grabbing transition-colors',
        isFromCatalog ? 'hover:bg-opacity-30' : 'hover:bg-opacity-10',
        isDragging && 'invisible' // Aseguramos que el original sea invisible durante el arrastre
      )}
    >
      <GripVertical className="h-3 w-3 opacity-50" style={{ color: currentTheme.colors.primary }} />
      <span className="truncate font-medium" style={{ color: currentTheme.colors.text }}>{patology.nombre}</span>
      ...
      {!isFromCatalog && onRemove && (
        ...
        className="ml-1 p-0.5 rounded-full transition-colors hover:bg-black/10 cursor-pointer"
...
<DndContext
      ...
      >
        <form id="heredo-fam-form" onSubmit={handleSubmit(onSubmit)} className="space-y-3">
          {/* Overlay personalizado para el arrastre */}
          <DragOverlay>
            {activeDragId ? (
              <div 
              ...
              >
                ...
                <span className="truncate font-medium">
                  {activeDragId.startsWith('patology-') 
