...
export function AppointmentsNew() {
  ...
  // Ya no usamos selectedPatient - mostramos todas las citas
  ...
  }, [dateRange, user, authLoading]); // Removido selectedPatient - ya no filtramos por paciente
  ...
  const fetchAppointments = async () => {
    ...
    try {
      ...
      // Ya no filtramos por paciente seleccionado - mostramos todas las citas
      // El filtro se aplicará solo mediante la búsqueda
  ...
  const confirmStatusChange = async (notes: string) => {
    ...
    console.log('Selected appointment:', selectedAppointmentForStatus?.id);
    console.log('New status ID:', newStatusId);
    
    if (!selectedAppointmentForStatus || newStatusId === null) {
      console.log('❌ Missing appointment or status ID');
      return;
    }

    try {
      console.log('🟢 Calling api.appointments.changeStatus...');
      await api.appointments.changeStatus(selectedAppointmentForStatus.id, newStatusId);
      console.log('✅ changeStatus completed successfully');
                    ...
                    <React.Fragment key={appointment.id}>
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-center" style={{ color: currentTheme.colors.text }}>
                          {appointment.consultorio_nombre || `Consultorio ${appointment.consultorio}`}
                        </td>
                        <td className="px-4 py-3 text-right">
                          <AppointmentActionMenu
                            ...
                            onViewDetails={() => navigate('/citas', { state: { viewOnly: true, appointmentId: appointment.id, selectedPatient: appointment.patients } })}
                            onEdit={() => navigate('/citas', { state: { editMode: true, appointmentId: appointment.id, selectedPatient: appointment.patients } })}
                            onReschedule={() => navigate('/citas', { state: { editMode: true, appointmentId: appointment.id, selectedPatient: appointment.patients } })}
                            onChangeStatus={(statusId) => handleChangeStatus(appointment, statusId)}
                            onLoadStatusTransitions={async () => {
                              const transitions = await api.appointments.getFilteredStatusOptions(appointment.estado);
                              return transitions;
                            }}
