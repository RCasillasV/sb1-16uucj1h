...
function useTemporaryMessage(message: string | null | undefined, duration = 10000) {
...
export function Login() {
  ...
  const [loading, setLoading] = useState(false);
    // const [loginAttempts, setLoginAttempts] = useState(0); // This logic can be improved later
  const [lockoutUntil] = useState<Date | null>(null);

  // Mensaje de éxito temporal
    const [successMessage] = useTemporaryMessage(location.state?.message as string | undefined);
      ...
      if (error) {
        if (error.message.includes('Email not confirmed')) {
          ...
          setError('email', { type: 'manual', message: 'Usuario o contraseña incorrectos' });
                              // const newAttempts = (loginAttempts) => loginAttempts + 1;
                    // const updatedAttempts = newAttempts(0); // Simplified for now
                    // if (updatedAttempts >= 5) setLockoutUntil(new Date(Date.now() + 60 * 1000));
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            <label className="flex items-center">
              <input
                type="checkbox"
                {...register('rememberMe')} defaultChecked={false}
