...
export function SomatometryFormNew({ 
  ...
  const [ageMonths, setAgeMonths] = useState(() => 
    somatometryService.calculateAgeInMonths(
  ...
  const [detailedAge, setDetailedAge] = useState(() =>
    somatometryService.calculateDetailedAge(
      patient.FechaNacimiento,
      formData.measurement_date
    )
  );

  // Efecto para actualizar la edad cuando cambia la fecha de medición
  useEffect(() => {
    const newAgeMonths = somatometryService.calculateAgeInMonths(
      patient.FechaNacimiento,
      formData.measurement_date
    );
    ...
    if (newAgeMonths < 0) {
      setDateError('La fecha de medición no puede ser anterior a la fecha de nacimiento del paciente');
      setAgeMonths(0);
      setDetailedAge({ years: 0, months: 0, days: 0, totalMonths: 0, totalDays: 0 });
      return;
    } else {
      setDateError(null);
    }
    
    setAgeMonths(newAgeMonths);
    
    const newDetailedAge = somatometryService.calculateDetailedAge(
      patient.FechaNacimiento,
      formData.measurement_date
    );
    setDetailedAge(newDetailedAge);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validar que no hay errores de fecha antes de enviar
    if (dateError) {
      setError('Por favor corrige la fecha de medición antes de continuar');
      setLoading(false);
      return;
    }
    
    // Validar que la edad es válida
    if (ageMonths < 0) {
      setError('La fecha de medición debe ser posterior a la fecha de nacimiento del paciente');
      setLoading(false);
      return;
    }
          ...
          <div className="flex-1">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setFormData(prev => ({ ...prev, measurement_date: newDate }));
                
                // Calcular la nueva edad para verificar si es válida
                const newAgeMonths = somatometryService.calculateAgeInMonths(
                  patient.FechaNacimiento,
                  newDate
                );
                
                // Solo actualizar slider ranges si la edad es válida (no negativa)
                if (newAgeMonths >= 0) {
                  somatometryService.getSliderRanges(
                    ...
                    console.error('Error updating slider ranges:', err);
                  });
                }
              }}
              ...
              style={{
                ...
                borderColor: dateError ? '#DC2626' : currentTheme.colors.border,
                height: '40px'
              }}
              required
              min={patient.FechaNacimiento}
