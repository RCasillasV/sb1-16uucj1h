// src/pages/AppointmentForm.tsx   Bolt  
...
import { DETAILED_APPOINTMENT_STATUSES, DetailedAppointmentStatus } from '../utils/appointmentStatuses';
...
export function AppointmentForm({ onSuccess, onCancel, appointment }: AppointmentFormProps) {
  ...
  // Usar el tipo DetailedAppointmentStatus directamente
  const [status, setStatus] = useState<DetailedAppointmentStatus>(
    (appointment?.estado as DetailedAppointmentStatus) || 'Programada'
  );
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const appointmentData: AppointmentInsert = {
      ...
      motivo: reason,
      notas: notes || null,
      // Enviar el estado directamente, ya que la DB lo soporta
      estado: status,
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            id="status"
            value={status}
            onChange={(e) => setStatus(e.target.value as DetailedAppointmentStatus)}
            className="flex-1 rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
            style={{
              backgroundColor: currentTheme.colors.surface,
              color: currentTheme.colors.text,
          ...
          >
            {Object.keys(DETAILED_APPOINTMENT_STATUSES).map((key) => (
              <option key={key} value={key}>
                {DETAILED_APPOINTMENT_STATUSES[key as DetailedAppointmentStatus].display}
              </option>
            ))}
          </select>
        </div>
