...
import { generatePdfThumbnailFile } from '../lib/pdfThumbnail';
...
interface UploadedFile {
  ...
  size: number;
  type: string;
  mimeType?: string; // Agregar para compatibilidad con procesamiento IA
  url: string;
  path: string;
}
  ...
  const uploadFile = async (file: File, description: string): Promise<UploadedFile> => {
    ...
    if (file.type.startsWith('image/')) {
      try {
        ...
        console.error('Error creating image thumbnail:', thumbnailError);
        // Continue without thumbnail if generation fails
      }
    } else if (file.type === 'application/pdf') {
      try {
        console.log(`FileUpload: Generando thumbnail para PDF ${file.name}...`);
        // Generate thumbnail from first page of PDF
        const thumbnailFile = await generatePdfThumbnailFile(file, 300, 400);
        ...
        if (!thumbnailError) {
          ...
          thumbnailUrl = thumbnailPublicUrl;
          console.log(`FileUpload: Thumbnail de PDF generado exitosamente`);
        }
      } catch (thumbnailError) {
        console.error('Error creating PDF thumbnail:', thumbnailError);
        // Continue without thumbnail if generation fails
      }
    }
    ...
    try {
      ...
      fileRecord = await api.files.create({
      ...
      console.log(`FileUpload: Metadata guardada exitosamente para ${file.name}`, fileRecord);
    } catch (dbError) {
    ...
    return {
      id: fileRecord?.id || data.path, // ID del registro de BD
      name: description, // Use description as the display name
      size: file.size,
      type: file.type,
      mimeType: file.type, // Agregar mimeType para compatibilidad
      url: publicUrl,
      path: filePath,
    };
  };
