...
export function PatientListSelector({ onSelectPatient, onClose, className = '', isModal = false, refreshTrigger }: PatientListSelectorProps) {
  ...
  // Debounce search term para evitar búsquedas en cada tecla
  ...
  }, [authLoading, refreshTrigger]); // Removido user para evitar re-renders innecesarios
  ...
  const filteredPatients = allPatients.filter(patient => 
    `${patient.Nombre} ${patient.Paterno} ${patient.Materno}`.toLowerCase().includes(debouncedSearchTerm.toLowerCase()) ||
    patient.Email?.toLowerCase().includes(debouncedSearchTerm.toLowerCase()) ||
    patient.Telefono?.includes(debouncedSearchTerm)
  );
