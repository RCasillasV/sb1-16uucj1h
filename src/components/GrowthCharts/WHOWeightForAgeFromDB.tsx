...
const WHOWeightForAgeFromDB: React.FC<WHOWeightForAgeFromDBProps> = ({ 
  records, 
  gender, 
  patientName 
}) => {
  const { currentTheme } = useTheme();
  const [whoData, setWhoData] = useState<{ ageMonths: number; p3: number; p50: number; p97: number; }[]>([]);
  ...
  if (isLoading) {
    return (
      <div className="w-full h-[690px] flex items-center justify-center">
  ...
  const chartData: ChartData<'line'> = {
    ...
    datasets: [
      ...
      ...(patientPoints.length > 0 ? [{
        label: patientName,
        data: patientPoints,
        borderColor: currentTheme.colors.primary,  // Color del tema para la línea
        backgroundColor: currentTheme.colors.primary,  // Color del tema para los puntos
        borderWidth: 4,  // Línea aún más gruesa
        pointRadius: 9,  // Puntos aún más grandes
        pointHoverRadius: 12,  // Efecto hover más pronunciado
        pointHoverBorderWidth: 4,  // Borde más grueso al pasar el cursor
        pointBorderColor: '#fff',
        pointBorderWidth: 2,  // Borde blanco más grueso
        tension: 0.2,
        fill: false
      }] : [])
    ]
  ...
  const options: ChartOptions<'line'> = {
    ...
    scales: {
      ...
      y: {
        ...
        max: 28,
        ticks: {
          stepSize: 1, // Un tick por kilo
          callback: (value) => `${value} kg`
        },
        grid: {
          color: (context) => {
            // Líneas más oscuras cada 5 kilos
            return context.tick.value % 5 === 0 ? '#A0A0A0' : '#D0D0D0';
          },
          lineWidth: (context) => {
            // Líneas más gruesas cada 5 kilos
  ...
  return (
    <div className="w-full">
      ...
      <div className="text-xs text-gray-400 mt-1 text-center">
        Estado: {isLoading ? '⏳ Cargando...' : error ? '❌ Error' : whoData ? '✅ Datos cargados' : '⚠️ Sin datos'}
      </div>
    </div>
  );
};
