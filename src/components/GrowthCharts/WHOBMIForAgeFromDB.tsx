...
interface WHOBMIForAgeFromDBProps {
  records: SomatometryRecord[];
  gender: 'M' | 'F';
  patientName: string;
}

export const WHOBMIForAgeFromDB: React.FC<WHOBMIForAgeFromDBProps> = ({
  records,
  gender,
  patientName
}) => {
  ...
  useEffect(() => {
    const loadWHOData = async () => {
      try {
        ...
        console.log('🔍 Cargando datos OMS IMC para:', { type: 'bmi', gender, genderCode });
        ...
        console.log('🔍 Consulta directa a tabla IMC:', { rawData, rawError, count: rawData?.length || 0 });
        
        // Usar datos completos de la BD directamente
        const completeData = await somatometryService.getCompleteWHOBMIData(genderCode);
        console.log('📊 Datos OMS IMC completos recibidos:', completeData);
        console.log('📈 Cantidad de puntos IMC:', completeData?.length || 0);
        
        if (!completeData?.length) {
          throw new Error('No hay datos OMS de IMC disponibles en la base de datos');
        }
        ...
        console.error('❌ Error loading WHO BMI data:', err);
    ...
    return (
      <div className="w-full h-[768px] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Cargando datos OMS IMC...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="w-full h-[768px] flex items-center justify-center">
        <div className="text-center text-red-600">
          <p className="text-lg font-semibold">Error al cargar gráfica de IMC</p>
  ...
  const options: ChartOptions<'line'> = {
    ...
    plugins: {
      title: {
        display: true,
        text: `IMC para la edad - ${isGirl ? 'NIÑAS' : 'NIÑOS'} (Datos OMS BD)`,
        font: { size: 18, weight: 'bold' },
        color: backgroundColor
      },
      legend: {
        position: 'top',
        align: 'end',  // Alinear al centro
        labels: { 
          usePointStyle: true, 
          padding: 10,
          font: { size: 11 }
    ...
    scales: {
      ...
      y: {
        ...
        grid: {
          color: (context) => {
            // Líneas más oscuras cada 5 unidades
            return context.tick.value % 5 === 0 ? '#A0A0A0' : '#D0D0D0';
          },
          lineWidth: (context) => {
            // Líneas más gruesas cada 5 unidades
...
export default WHOBMIForAgeFromDB;
