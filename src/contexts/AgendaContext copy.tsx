...
export function AgendaProvider({ children }: { children: React.ReactNode }) {
  console.log('AgendaProvider: Initializing');
  const { user, loading: authLoading } = useAuth();
  console.log('AgendaProvider: useAuth hook called, user:', user, 'loading:', authLoading);
  ...
  // Cargar configuración cuando el usuario esté disponible y no esté cargando
  useEffect(() => {
    if (!authLoading && user) {
      loadConfiguration();
    }
  ...
  const isWorkDay = useCallback((date: string): boolean => {
    ...
    const dayOfWeek = dateObj.toLocaleDateString('es-ES', { weekday: 'long', timeZone: 'UTC' });
    const capitalizedDay = dayOfWeek.charAt(0).toUpperCase() + dayOfWeek.slice(1);
    
    console.log(`isWorkDay: Checking date ${date}. Day of week (raw): "${dayOfWeek}", Capitalized: "${capitalizedDay}". Configured days:`, agendaSettings.consultation_days);
